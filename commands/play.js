...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!channel) return message.reply(" **You need to join a voice channel first!** :notes: ").catch(console.error);
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return message.reply(`You must be in the same channel as ${message.client.user}`).catch(console.error);

    if (!args.length)
      return message
        .reply(`:x: Usage: ${message.client.prefix}play <YouTube URL | Video Name | Soundcloud URL> :slight_smile: `)
        .catch(console.error);
